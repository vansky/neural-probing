...
parser.add_argument('--classifier_file', type=str,  default='classifier.pt',
                    help='path to save the final classifier')
...
# a bptt-limit of 2, we'd get the following two Tensors for i = 0:
#  a g m s      b h n t
#  b h n t      c i o u
...
def get_classifier_batch(source, class_source, i):
    seq_len = min(args.bptt, len(source) - 1 - i)
    data = source[i:i+seq_len]
    target = class_source[i:i+seq_len].view(-1)
    return data, target
...
def evaluate_classifier(data_source,class_data_source):
    # Turn on evaluation mode which disables dropout.
    model.eval()
    classifier.eval()
    total_loss = 0.
...
if not args.test and not args.interact:
    ...
    try:
        for epoch in range(1, args.epochs+1):
            epoch_start_time = time.time()
            train_classifier()
            val_loss = evaluate_classifier(val_data,val_class_data)
            print('-' * 89)
            print('| end of epoch {:3d} | time: {:5.2f}s | valid loss {:5.2f} | '
                  'valid ppl {:8.2f}'.format(epoch, (time.time() - epoch_start_time),
                                             val_loss, math.exp(val_loss)))
            ...
            if not best_val_loss or val_loss < best_val_loss:
                ...
                # Anneal the learning rate if no improvement has been seen in the validation dataset.
                if val_loss == prev_val_loss == prev2_val_loss:
                    print('Covergence achieved! Ending classifier training early')
                    break
                prev2_val_loss = prev_val_loss
                prev_val_loss = val_loss
                lr /= 4.0
